...
Almost none of the code was typed: roughly 99% of it was generated by
AI agents, and my work went into the requirements, the design, the
decisions the agents asked about, and reviewing and editing what they produced.

- **Claude Code** drafted the PRDs ([docs/PRD-P0.md](docs/PRD-P0.md),
  [docs/PRD-P1.md](docs/PRD-P1.md)), reviewed and edited by hand.
  The prompts that produced them and the rest of the project are logged in
  [docs/prompts.md](docs/prompts.md).
- **Claude Design** produced the UI and the design system
  (`packages/ui/src/styles/`, `.agents/skills/design-system/`).
- **Agentic Workflow Creator**, a tool created by me [here](https://github.com/rimbener/agentic-workflow-creator), it's a tool to create and ran two workflows defined in
  [`workflows/`](workflows/):
  - `prd-to-spec` interviews the user about a group of PRD requirements and
    turns the answers into an approved, committed spec.
  - `spec-to-code` builds each spec into code by TDD, slice by slice, then
    reviews and edits the result. The output was reviewed and edited by hand
    before the task was closed. Mutation testing was left out of the loop to
    keep each run short.

  Every spec, interview log, review and verdict the workflows produced is kept
  under `.awc/tasks/done/` as the project's audit trail.
...
| `npm run test:coverage` | Every suite once, with a coverage report per workspace |
...
Vitest everywhere; React Testing Library under jsdom for the two React
workspaces. `npm test` runs every suite once; `npm run test:coverage` runs
them again with V8 coverage and writes a report into each workspace's
`coverage/` folder (`coverage/index.html` is the browsable one; the text
summary prints in the terminal). The folders are gitignored, so regenerate
them locally. Latest run, 525 tests, all green:

| Workspace                | Tests | Statements | Branches | Functions | Lines |
| ------------------------ | ----- | ---------- | -------- | --------- | ----- |
| `packages/contracts`     | 100   | 100%       | 100%     | 100%      | 100%  |
| `apps/calc-service`      | 93    | 100%       | 94.7%    | 100%      | 100%  |
| `apps/api-gateway`       | 89    | 100%       | 97.1%    | 100%      | 100%  |
| `apps/sezzle-calculator` | 150   | 100%       | 100%     | 100%      | 100%  |
| `packages/ui`            | 93    | 90.7%      | 98.0%    | 82.1%     | 89.8% |

What each layer's tests pin down: every operation and edge case in
calc-service (division by zero, negative square root, overflow to a
non-finite result); validation, proxying, the timeout and the retry in the
gateway; entry rules, results and error rendering in the SPA; the contract's
schemas, codes and messages. Each workspace README lists its suites.
...
- **TypeScript on the backend, not Go.** The brief prefers Go, and I have no
  experience with it. Rather than learn a language inside an assessment's
  time budget and hand in code I could not review with confidence, I chose
  the stack I can judge: both services are TypeScript on Hono + Zod. That
  choice also lets one workspace package,
  [`@repo/contracts`](packages/contracts/README.md), own the request and
  response schemas, the error codes and messages, and each code's HTTP status
  — and all three apps import that package as source. Both services and the
  SPA validate with the same Zod schema, so the contract cannot drift between
  layers, and one toolchain (one lint, one test runner, one `tsc`) covers the
  whole repo. The cost is the language preference. The boundaries make it
  recoverable: the services talk only over HTTP, calc-service is internal and
  has one route, so either service could be rewritten in Go behind the same
  contract without touching the SPA.
