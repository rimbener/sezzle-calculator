...
import { GATEWAY_URL_VARIABLE } from './api/client'
...
describe('the frontend computes nothing (AC-24)', () => {
  ...
  it('names the gateway URL env var in the API client alone (AC-7)', () => {
    const named = files.filter(file => source(file).includes(GATEWAY_URL_VARIABLE)).map(file => relative(__dirname, file))

    expect(named).toEqual([relative(__dirname, API_CLIENT)])
  })
